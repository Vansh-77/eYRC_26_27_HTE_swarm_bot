...
def find_trapezoids(frame):
    ...
    ##############  ADD YOUR CODE HERE  ##############
    img = frame[ARENA_Y0:ARENA_Y1, ARENA_X0:ARENA_X1]
    ...
    floor_color = np.array([l, a, b], dtype=np.float32)

    difference = lab.astype(np.float32) - floor_color
    ...
    while rclpy.ok():
        ...
        if trapezoids and now - last_report >= REPORT_PERIOD_SEC:
            ...
            for cx, cy, _ in sorted(trapezoids, key=lambda t: (t[1], t[0])):
                ...
                client_asyn = client.call_async(request)

                rclpy.spin_until_future_complete(node, client_asyn, timeout_sec=1.0)

                if not client_asyn.done():
                    print("Time out")
                else:
                    response = client_asyn.result()
